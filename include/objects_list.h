...
template <class X, class T>
class objects_list : public virtual tecnoballz {
 protected:
  static T* singleton;
  X** sprites_list;
...
template <class X, class T>
T* objects_list<X, T>::singleton = NULL;

/**
 * Create the list of objects
 */
template <class X, class T>
objects_list<X, T>::objects_list() {}
...
template <class X, class T>
T* objects_list<X, T>::get_instance() {
  if (NULL == singleton) {
    singleton = new T();
  }
  return singleton;
 ...
 */
template <class X, class T>
void objects_list<X, T>::littleInit() {
  object_init();
  max_of_sprites = 0;
 ...
 */
template <class X, class T>
void objects_list<X, T>::release_sprites_list() {
 ...
 */
template <class X, class T>
X** objects_list<X, T>::get_sprites_list() {
  return sprites_list;
}
 ...
 */
template <class X, class T>
X* objects_list<X, T>::get_first_sprite() {
  if (NULL == sprites_list) {
    return NULL;
  }
 ...
 */
template <class X, class T>
Uint32 objects_list<X, T>::get_max_of_sprites() {
 ...
 */
template <class X, class T>
void objects_list<X, T>::set_max_of_sprites(Uint32 maxof) {
 ...
 */
template <class X, class T>
void objects_list<X, T>::alloc_sprites_list() {
  if (0 == max_of_sprites) {
    std::cerr << "(!)objects_list::alloc_sprites_list() "
              << "Our array should always have at least one element in it!" << std::endl;
    throw(
        "(!)objects_list::alloc_sprites_list() failed! "
  ...
  try {
    ...
    std::cerr << "(!)objects_list::alloc_sprites_list() "
              << "not enough memory to allocate " << max_of_sprites << " elements!" << std::endl;
    throw;
  }
 ...
 */
template <class X, class T>
void objects_list<X, T>::create_sprites_list(Uint32 maxof, bool have_shades) {
  max_of_sprites = maxof;
 ...
 */
template <class X, class T>
void objects_list<X, T>::create_sprites_list() {
  alloc_sprites_list();
  ...
  sprite_template->create_sprite(sprite_type_id, sprites_bitmap, sprites_have_shades,
 ...
 */
template <class X, class T>
void objects_list<X, T>::enable_sprites() {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    X* sprite = sprites_list[i];
    sprite->enable();
  }
 ...
 */
template <class X, class T>
void objects_list<X, T>::disable_sprites() {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    X* sprite = sprites_list[i];
    sprite->disable();
  }
