...
handler_high_score::handler_high_score() {
  ...
  scores_table_size =
      (sizeof(Uint32) * 3 + 8) * MAX_OF_HIGH_SCORES * MAX_OF_DIFFICULTY_LEVELS + sizeof(Uint32);
  first_init();
}
 ...
 */
handler_high_score* handler_high_score::get_instance() {
 ...
 */
void handler_high_score::first_init() {
  try {
...
bool handler_high_score::load_high_score() {
  ...
  if (file_size != scores_table_size) {
    std::cerr << "(!)handler_high_score::load_high_score() "
              << " bad file size, " << file_size << " byte(s) instead" << scores_table_size
              << "bytes!" << std::endl;
    delete[] file_data;
    return false;
  }
  ...
  Uint32 calculated_sum =
      calculate_checksum(ptr32 + 1, (scores_table_size - sizeof(Uint32)) / sizeof(Uint32));
  if (calculated_sum != read_sum) {
    std::cerr << "(!)handler_high_score::load_high_score() "
              << " bad checksum, " << read_sum << " instead " << calculated_sum << "!"
              << std::endl;
    delete[] file_data;
    return false;
  }
 ...
 * Save the high score
 */
void handler_high_score::save_high_score() {
  char* file_data;
  try {
    file_data = new char[scores_table_size];
  } catch (std::bad_alloc&) {
    std::cerr << "(!)handler_high_score::save_high_score() "
              << "not enough memory to allocate " << scores_table_size << "bytes!" << std::endl;
    throw;
  }
 ...
 */
Uint32 handler_high_score::calculate_checksum(Uint32* addr, Uint32 data_size) {
  Uint32 value = 0;
...
bool handler_high_score::is_player_ranked() {
  bool is_ranked = is_player_ranked(&current_player->player_name[0], current_player->score_value,
                                    current_player->level_number, current_player->area_number);
  if (is_ranked) {
    sort_scores();
    save_high_score();
  }
 ...
 * @return true if the player score is ranked, otherwise false
 */
bool handler_high_score::is_player_ranked(char* playername,
                                          Uint32 score_value,
                                          Uint32 level_num,
                                          Uint32 area_num) {
  /* exit if cheat mode! */
    ...
    if (score_value > score[i].value) {
      ...
      if (is_verbose) {
        std::cout << "handler_high_score::is_player_ranked() "
                  << "rank:" << i << " player name:" << score[i].player_name << std::endl;
      }
      return true;
    }
  }
 ...
 * Sort scores table
 */
void handler_high_score::sort_scores() {
  if (is_verbose) {
 ...
 */
player_score* handler_high_score::get_high_score_table() {
 ...
 */
char* handler_high_score::get_best_playername() {
  player_score* score = scores_tables[difficulty_level - 1];
