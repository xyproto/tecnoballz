 ...
 */
void tecnoballz::first_init(configfile* pConf) {
  config_file = pConf;
  if (is_verbose) {
 ...
 */
void tecnoballz::game_begin() {
  supervisor* stage = NULL;
  do {
    if (is_verbose) {
      std::cout << ">tecnoballz::game_begin() phase:" << current_phase << std::endl;
    }
    switch (current_phase) {
      ...
      case MAP_EDITOR:
        ...
        break;
      default:
        std::cerr << "(!)tecnoballz::game_begin() phase number " << current_phase << "is invalid!"
                  << std::endl;
 ...
 */
void tecnoballz::release_all_objects(configfile* pConf) {
 ...
 */
void tecnoballz::integer_to_ascii(Sint32 value, Uint32 padding, char* str) {
...
void tecnoballz::int_to_big_endian(Uint32* ptsrc, Uint32* ptdes) {
...
void tecnoballz::big_endian_to_int(Uint32* ptsrc, Uint32* ptdes) {
#if SDL_BYTEORDER == SDL_BIG_ENDIAN
  *ptdes = *ptsrc;
#else
  char* s = (char*)ptsrc;
