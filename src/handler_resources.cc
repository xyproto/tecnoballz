...
handler_resources* handler_resources::handler_resources_singleton = NULL;
...
char handler_resources::tmp_filename[512];
char handler_resources::pathstring[512];

const char* handler_resources::standfiles[] = {
    "cosinus128.list",  // RESCOSLIST
    /* MAP_GUARDIANS_20 */
    "tilemap-guardians_20.data", "tilemap-guardians_40.data",
    /* MAP_CONGRATULATIONS_20 */
    "tilemap-congratulation_20.data", "tilemap-congratulation_40.data",
    /* MAP_MENU_20 */
    "tilemap-menu_20.data", "tilemap-menu_40.data",
...
const char* handler_resources::musicfiles[] = {"area1-game2.mod",
                                               "area2-game.mod",
                                               "area3-game.mod",
                                               "area4-game.mod",
                                               "area5-game.mod",
...
const char* handler_resources::soundfiles[] = {
    "rlife_moins.wav",  // 01
    "rexplo_rak.wav",     "rlife_plus.wav", "rmoney.wav",
    "ralarm.wav",  // 05
    "rgadget_sound.wav",  "rtecno.wav",     "rappar.wav",         "rtransfo.wav",
    "rtir_monstre.wav",  // 10
    "rtir_rak.wav",       "rexplo_big.wav", "rdetruit_indes.wav", "rexeplo_atom.wav",
    "rmontre_touche.wav",  // 15
    "ratom.wav",          "rindes_1.wav",   "rindes_2.wav",       "rraquette.wav",
    "rbricote.wav",  // 20
    "rbrique1.wav",       "rbrique2.wav",   "rbrique3.wav",       "rbrique4.wav",
    "rbrique5.wav",  // 25
    "raspire.wav",
    "reject.wav",  // 27
};
 ...
 */
handler_resources* handler_resources::get_instance() {
 ...
 */
char* handler_resources::load_data(Uint32 resource_id) {
 ...
 */
char* handler_resources::get_filename(Uint32 resource_id, Uint32 res) {
  const char* pfile;
 ...
 */
char* handler_resources::get_music_filename(Uint32 resource_id) {
  const char* pfile;
 ...
 */
char* handler_resources::get_sound_filename(Uint32 resource_id) {
 ...
 */
char* handler_resources::get_tilemaps_filename(Uint32 title_num) {
 ...
 */
char* handler_resources::get_full_pathname(Uint32 resource_id) {
 ...
 */
char* handler_resources::locate_data_file(const char* const name) {
  /*
    if (is_verbose)
      {
        std::
  ...
  std::cerr << "(!)handler_resources::locate_data_file() file '" << name << "' not found!"
            << std::endl;
  throw std::ios_base::failure(std::string("[!]handler_resources::locate_data_file() File '") +
 ...
 */
void handler_resources::load_sprites_bitmap(Uint32 resource_id) {
  release_sprites_bitmap();
 ...
 */
void handler_resources::release_sprites_bitmap() {
 ...
 */
char** handler_resources::load_texts(Uint32 resource_id,
                                     Uint32 numof_lines,
                                     Uint32 row_length,
                                     Uint32 modulo,
                                     bool upper_case) {
  ...
  if (numof_lines > 0 && numof_lines != list_count) {
    std::cerr << "(!)handler_resources::load_texts() " << numof_lines << " exceptes lines, read "
              << list_count << " lines!" << std::endl;
    throw std::runtime_error(
        "(!))handler_resources::load_texts() "
        "bad number of lines!");
  }
  ...
  try {
    ...
    std::cerr << "(!)handler_resources::load_texts() "
              << "not enough memory to allocate " << alloc_size << " bytes!" << std::endl;
    throw;
  }
 ...
 */
char* handler_resources::loadfile_with_lang(const char* const filename, Uint32* const fsize) {
 ...
 */
char* handler_resources::load_file(const char* fname) {
 ...
 * return a pointer to the file data
 */
char* handler_resources::load_file(const char* fname, Uint32* fsize) {
  /* locate a file under one of the data directories */
  ...
  if (fhand == -1) {
    std::cerr << "(!)handler_resources::load_file() "
              << "can't open file " << fname << "; error: " << strerror(errno) << std::endl;
    throw std::ios_base::failure(
  ...
  if (fstat(fhand, &sb)) {
    std::cerr << "(!)handler_resources::load_file() "
              << "can't stat file " << fname << "strerror:" << strerror(errno) << std::endl;
    throw std::ios_base::failure(
        "(!)handler_resources::load_file() "
        "can't stat a file!");
  }
  ...
  try {
    ...
    std::cerr << "(!)handler_resources::load_file() "
              << "not enough memory to allocate " << sb.st_size << " bytes!" << std::endl;
    throw;
  }
  ...
  if (read(fhand, buffer, sb.st_size) != sb.st_size) {
    std::cerr << "(!)handler_resources::load_file() "
              << "can't read file " << fname << "strerror:" << strerror(errno) << std::endl;
    throw std::ios_base::failure(
        "(!)handler_resources::load_file() "
        "can't read a file!");
    return NULL;
 ...
 */
Uint32 handler_resources::get_filesize_loaded() {
  return last_filesize_loaded;
}

void handler_resources::set_filesize_loaded(Uint32 size) {
  Uint32 i = size;
  i++;
  last_filesize_loaded = size;
}

char* handler_resources::read_complete_file(const char* filename) {
  std::ifstream file(filename, std::ios::ate);
  if (!file.is_open()) {
    std::cerr << "(!)handler_ressources::read_complete_file()"
                 "can't open file '"
              << filename << "'" << std::endl;
    // throw std::ios_base::failure ("(!)handler_ressources::read_complete_file() can't read a
    // file!");
    return NULL;
  }
  ...
  try {
    ...
    std::cerr << "(!)handler_resources::read_complete_file() "
              << "not enough memory to allocate " << size << " bytes!" << std::endl;
    throw;
  }
 ...
 */
void handler_resources::load_sinus() {
 ...
 * load scores table
 */
char* handler_resources::load_high_score_file() {
  char* filedata = NULL;
  try {
 ...
 * @pram size
 */
void handler_resources::save_high_score_file(char* buffer, Uint32 size) {
  size_t left;
  ...
  if (fhand == -1) {
    fprintf(stderr, "handler_resources::save_high_score_file(): file:%s / error:%s\n", fnamescore,
            strerror(errno));
  }
  left = size;
  while (left > 0) {
  ...
  if (close(fhand) == -1) {
    ...
    if (is_verbose)
      fprintf(stdout,
              "handler_resources::save_high_score_file(): "
              "file:%s size:%i\n",
              fnamescore, size);
  }
}
 ...
 */
const Sint16 handler_resources::cosinus360[720] = {
    ...
    43,   41,   39,   37,   35,   33,   31,   29,   26,   24,   22,   20,   18,   16,   13,   11,
    9,    7,    4,    2,    0,    -2,   -4,   -7,   -9,   -11,  -13,  -15,  -18,  -20,  -22,  -24,
    -26,  -29,  -31,  -33,  -35,  -37,  -39,  -41,  -43,  -45,  -48,  -50,  -52,  -54,  -56,  -58,
