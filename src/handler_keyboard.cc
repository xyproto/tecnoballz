...
Uint32 handler_keyboard::key_codes[K_MAXOF] = {SDLK_LEFT, SDLK_RIGHT,  SDLK_UP,
                                               SDLK_DOWN, SDLK_SPACE,  SDLK_LCTRL,
                                               SDLK_LALT, SDLK_PAGEUP, SDLK_PAGEDOWN};
 ...
 */
void handler_keyboard::init_joysticks() {
  joy_fire = false;
  joy_release = false;
  joy_gigablitz = false;
  joy_option = false;
    ...
    for (Uint32 i = 0; i < numof_joysticks; i++) {
      ...
      if (sdl_joysticks[i] == NULL) {
        std::cerr << "(!)handler_keyboard::init_joysticks()"
                  << " couldn't open joystick " << i << ": " << SDL_GetError() << std::endl;
      } else {
        if (is_verbose) {
 ...
 */
handler_keyboard* handler_keyboard::get_instance() {
...
void handler_keyboard::set_grab_input(bool) {
  return;
}
#else
void handler_keyboard::set_grab_input(bool mode) {
  if (mode) {
    is_grab_input = true;
...
void handler_keyboard::handle_console_buttons(SDL_Event* event) {
#ifdef TECNOBALLZ_GP2X
  ...
  if (event->type == SDL_JOYBUTTONDOWN) {
    ...
    if (event->jbutton.button == GP2X_BUTTON_UP || event->jbutton.button == GP2X_BUTTON_UPRIGHT ||
    ...
    if (event->jbutton.button == GP2X_BUTTON_LEFT || event->jbutton.button == GP2X_BUTTON_UPLEFT ||
        event->jbutton.button == GP2X_BUTTON_DOWNLEFT || event->jbutton.button == GP2X_BUTTON_L) {
      set_joy(IJOY_LEFT);
    }

    if (event->jbutton.button == GP2X_BUTTON_RIGHT ||
        event->jbutton.button == GP2X_BUTTON_UPRIGHT ||
        event->jbutton.button == GP2X_BUTTON_DOWNRIGHT || event->jbutton.button == GP2X_BUTTON_R) {
      set_joy(IJOY_RIGHT);
    }
    ...
    if (event->jbutton.button == GP2X_BUTTON_X || event->jbutton.button == GP2X_BUTTON_Y) {
      set_joy(IJOY_OPTION);
    }
    ...
    if (event->jbutton.button == GP2X_BUTTON_SELECT && last_command_keys[TOGGLE_POPUP_MENU]) {
      toggle_popup_menu();
    }
    if (event->jbutton.button == GP2X_BUTTON_START && last_command_keys[COMMAND_KEY_PAUSE]) {
      toggle_pause();
    }
    if (event->jbutton.button == GP2X_BUTTON_VOLUP && last_command_keys[VOLUME_UP]) {
      last_command_keys[VOLUME_UP] = false;
      command_keys[VOLUME_UP] = true;
    }
    if (event->jbutton.button == GP2X_BUTTON_VOLDOWN && last_command_keys[VOLUME_DOWN]) {
  ...
  /* Directions (UDLR) */
  joy_top = gp2x_buttons[GP2X_BUTTON_UP] | gp2x_buttons[GP2X_BUTTON_UPLEFT] |
            gp2x_buttons[GP2X_BUTTON_UPRIGHT];
  joy_down = gp2x_buttons[GP2X_BUTTON_DOWN] | gp2x_buttons[GP2X_BUTTON_DOWNLEFT] |
             gp2x_buttons[GP2X_BUTTON_DOWNRIGHT];
  joy_left = gp2x_buttons[GP2X_BUTTON_LEFT] | gp2x_buttons[GP2X_BUTTON_UPLEFT] |
             gp2x_buttons[GP2X_BUTTON_DOWNLEFT] | gp2x_buttons[GP2X_BUTTON_L];
  joy_right = gp2x_buttons[GP2X_BUTTON_RIGHT] | gp2x_buttons[GP2X_BUTTON_UPRIGHT] |
  ...
  /* Quit */
  command_keys[QUIT_TECNOBALLZ] =
      gp2x_buttons[GP2X_BUTTON_CLICK] & gp2x_buttons[GP2X_BUTTON_START];
#endif
#ifdef TECNOBALLZ_PSP
  ...
  if (event->type == SDL_JOYBUTTONDOWN) {
    ...
    if (event->jbutton.button == PSP_BUTTON_SELECT && last_command_keys[TOGGLE_POPUP_MENU]) {
      toggle_popup_menu();
    }
    if (event->jbutton.button == PSP_BUTTON_START && last_command_keys[COMMAND_KEY_PAUSE]) {
      toggle_pause();
    }
  }
 ...
 */
void handler_keyboard::toggle_popup_menu() {
  last_command_keys[TOGGLE_POPUP_MENU] = false;
  command_keys[TOGGLE_POPUP_MENU] = command_keys[TOGGLE_POPUP_MENU] ? false : true;
 ...
 */
void handler_keyboard::toggle_pause() {
  last_command_keys[COMMAND_KEY_PAUSE] = false;
  if (!command_keys[TOGGLE_POPUP_MENU]) {
    command_keys[COMMAND_KEY_PAUSE] = command_keys[COMMAND_KEY_PAUSE] ? false : true;
    if (is_grab_input && command_keys[COMMAND_KEY_PAUSE]) {
      SDL_WM_GrabInput(SDL_GRAB_OFF);
    }
    if (is_grab_input && !command_keys[COMMAND_KEY_PAUSE]) {
 ...
 */
void handler_keyboard::read_events() {
  ...
  while (SDL_PollEvent(&event) > 0) {
    switch (event.type) {
      ...
      case SDL_KEYDOWN: {
        ...
        if (keys[SDLK_RALT] == SDL_RELEASED && keys[SDLK_LALT] == SDL_RELEASED &&
            keys[SDLK_LSHIFT] == SDL_RELEASED && keys[SDLK_RSHIFT] == SDL_RELEASED) {
          if (keys[SDLK_RCTRL] == SDL_RELEASED && keys[SDLK_LCTRL] == SDL_RELEASED) {
      ...
      case SDL_KEYUP: {
        ...
        /* enable pause [P] key */
        if (keys[SDLK_p] == SDL_RELEASED && last_command_keys[COMMAND_KEY_PAUSE]) {
          toggle_pause();
        }

        /* enable context menu [ESC] key */
        if (keys[TOGGLE_POPUP_MENU] == SDL_RELEASED && last_command_keys[TOGGLE_POPUP_MENU]) {
          toggle_popup_menu();
        }
        ...
        {
          if (keys[SDLK_ESCAPE] == SDL_RELEASED && last_command_keys[QUIT_TECNOBALLZ]) {
            last_command_keys[QUIT_TECNOBALLZ] = false;
            command_keys[QUIT_TECNOBALLZ] = true;
          }
          if (keys[SDLK_x] == SDL_RELEASED && last_command_keys[CAUSE_GAME_OVER]) {
            last_command_keys[CAUSE_GAME_OVER] = false;
            command_keys[CAUSE_GAME_OVER] = true;
          }
          if (keys[SDLK_q] == SDL_RELEASED && last_command_keys[QUIT_TO_MAIN_MENU]) {
            last_command_keys[QUIT_TO_MAIN_MENU] = false;
            command_keys[QUIT_TO_MAIN_MENU] = true;
          }
          if (keys[SDLK_f] == SDL_RELEASED && last_command_keys[TOGGLE_SOUND]) {
            last_command_keys[TOGGLE_SOUND] = false;
            command_keys[TOGGLE_SOUND] = true;
          }
          if (keys[SDLK_s] == SDL_RELEASED && last_command_keys[TOGGLE_AUDIO]) {
            last_command_keys[TOGGLE_AUDIO] = false;
            command_keys[TOGGLE_AUDIO] = true;
          }
          if (keys[SDLK_d] == SDL_RELEASED && last_command_keys[TOGGLE_MUSIC]) {
            last_command_keys[TOGGLE_MUSIC] = false;
            command_keys[TOGGLE_MUSIC] = true;
          }
          if (keys[SDLK_l] == SDL_RELEASED && last_command_keys[DISABLE_TIMER]) {
            last_command_keys[DISABLE_TIMER] = false;
            command_keys[DISABLE_TIMER] = true;
          }
          if (keys[SDLK_PAGEDOWN] == SDL_RELEASED && last_command_keys[VOLUME_DOWN]) {
            last_command_keys[VOLUME_DOWN] = false;
            command_keys[VOLUME_DOWN] = true;
          }
          if (keys[SDLK_PAGEUP] == SDL_RELEASED && last_command_keys[VOLUME_UP]) {
            last_command_keys[VOLUME_UP] = false;
            command_keys[VOLUME_UP] = true;
          }
        }
 ...
 */
void handler_keyboard::clear_command_keys() {
 ...
 */
Sint32 handler_keyboard::get_mouse_y_offset() {
 ...
 */
void handler_keyboard::set_key_code_down(Uint32 kcode) {
  if (kcode != SDLK_LSHIFT && kcode != SDLK_RSHIFT && kcode != SDLK_LCTRL && kcode != SDLK_RCTRL)
    key_code_down = kcode;
}
 ...
 * @param kcode key code of the pressed key
 */
void handler_keyboard::set_keycode_up(Uint32 kcode) {
  if (kcode != SDLK_LSHIFT && kcode != SDLK_RSHIFT && kcode != SDLK_LCTRL && kcode != SDLK_RCTRL) {
    code_keyup = kcode;
 ...
 */
void handler_keyboard::set_joy(Uint32 code) {
  joy_code_down = code;
  input_joy_tempo = 0;
}
 ...
 */
void handler_keyboard::clr_joy(Uint32 code) {
  joy_up = code;
 ...
 * Handle string input
 */
void handler_keyboard::input_string() {
  Sint32 kcode = 0;
  if (key_delay < 1) {
    kcode = key_code_down;
    if (key_code_down > 0) {
 ...
 */
void handler_keyboard::input_string(Uint32 kcode) {
  Sint32 j, i;
  if (0 == kcode) {
    return;
  }
  ...
  switch (kcode) {
    ...
    default:
       ...
       * A-Z (65 to 90)
       */
      if ((kcode >= ' ' && kcode <= '!') || (kcode >= '-' && kcode <= '.') ||
          (kcode >= '0' && kcode <= ':') || (kcode >= 'A' && kcode <= 'Z') || kcode == '\'') {
 ...
 */
void handler_keyboard::set_input_string(char* strng, Uint32 ssize) {
  string_cursor_pos = 0;
 ...
 */
void handler_keyboard::stop_string_input() {
  string_cursor_pos = 0;
  string_input_size = 0;
  current_input_string = NULL;
}
...
bool handler_keyboard::wait_key() {
  if (is_key_waiting) {
    if (!control_is_pressed(K_FIRE) && !control_is_pressed(K_RELEASE_BALL) &&
