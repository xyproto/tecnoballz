 ...
 */
void handler_levels::check_levels() {
  for (Uint32 i = 0; i < levels_counter; i++) {
    if (BRICKS_LEVEL == levels_list[i].type) {
      ...
      cerr << "(!)handler_levels::check_levels() '" << levels_list[i].type
 ...
 */
void handler_levels::check_xml(TiXmlNode* parent, Uint32 node) {
  if (NULL == parent) {
    return;
  }
      ...
      if (element == "capsules") {
        if (capsules_lists_counter > 0) {
          if (capsules_counter != MAX_OF_CASPULES) {
            cerr << "(!)handler_levels::check_xml() " << MAX_OF_CASPULES << " <id> childnodes "
                 << " of <capsules> chilnodes expected."
                 << " But " << capsules_counter << " found!" << endl;
            throw runtime_error(
                "(!)handler_levels::check_xml()"
 ...
 * @param node node indentifier of the seconde level
 */
void handler_levels::parse(TiXmlNode* parent, Uint32 node) {
  if (NULL == parent) {
    return;
  }
  Sint32 type = parent->Type();
  ...
  switch (type) {
      ...
      if (last_element == "level") {
        ...
        while (NULL != attribute) {
          ...
          if (attribute->QueryIntValue(&value) != TIXML_SUCCESS) {
            cerr << "(!)handler_levels::parse() '" << name << "' attribute is not a integer!"
                 << endl;
            throw runtime_error(
                "(!)handler_levels::parse()"
                "an attribute is not a integer!");
          }
          if (value < 0) {
            cerr << "(!)handler_levels::parse() '" << name << "' attribute is a negative integer!"
                 << endl;
            throw runtime_error(
                "(!)handler_levels::parse()"
          ...
          if ("type" == name) {
            if (value >= MAX_OF_LEVEL_TYPES) {
              cerr << "(!)handler_levels::parse() '" << value << "' is a invalid level type!"
                   << endl;
              throw runtime_error(
                  "(!)handler_levels::parse()"
    ...
    case TiXmlNode::TINYXML_TEXT:
      ...
      if (!sscanf(value_str.c_str(), "%d", &value)) {
        cerr << "(!)handler_levels::parse() '" << last_element << "' element is not a integer!"
             << endl;
        throw runtime_error(
            "(!)handler_levels::parse()"
      ...
      switch (node) {
        ...
        case GUARDIANS_LEVEL_NODE:
          ...
          if (last_element == "capsules_frequency") {
            guardians_levels[guardians_level_index].capsules_frequency = value;
          }
 ...
 */
guardians_level_desc* handler_levels::get_guardians_level(Uint32 id) {
  ...
  throw runtime_error(
      "(!)handler_levels::get_guardians_level()"
      "ID not found!");
}
 ...
 */
bricks_level_desc* handler_levels::get_bricks_level(Uint32 id) {
  ...
  cerr << "(!)handler_levels::get_bricks_level() "
       << "id " << id << " not found!" << endl;
  throw runtime_error(
      "(!)handler_levels::get_bricks_level()"
      "ID not found!");
}
 ...
 */
Uint32* handler_levels::get_capsules_list(Uint32 id) {
  ...
  cerr << "(!)handler_levels::get_capsules_list() "
       << "id " << id << " not found!" << endl;
  throw runtime_error(
      "(!)handler_levels::get_capsules_list()"
      "ID not found!");
}
 ...
 */
const bricks_level_desc* handler_levels::get_bricks_levels_params(Uint32 area_num,
                                                                  Uint32 level_num) {
  if (area_num < 1 || area_num > 5) {
    cerr << "(!)handler_levels::get_bricks_levels_params() "
         << "area_num:" << area_num << "out of range!" << endl;
    area_num = 1;
  }
  if (level_num < 1 || level_num > NUM_OF_LEVELS_PER_AREA) {
    cerr << "(!)handler_levels::get_bricks_levels_params() "
         << "level:" << level_num << "out of range!" << endl;
    level_num = 1;
  }
  if (level_num == 6 || level_num == NUM_OF_LEVELS_PER_AREA) {
    cerr << "(!)handler_levels::get_bricks_levels_params() "
         << "level:" << level_num << " is a guardians level!" << endl;
    level_num--;
  }
 ...
 * @return a pointer to a guardians level
 */
const guardians_level_desc* handler_levels::get_guardians_levels_params(Uint32 area_num,
                                                                        Uint32 level_num) {
  if (area_num < 1 || area_num > 5) {
  ...
  if (level_num != 6 && level_num != NUM_OF_LEVELS_PER_AREA &&
    ...
    cerr << "(!)handler_levels::get_guardians_levels_params() "
         << "level:" << level_num << " is a bricks level!" << endl;
    level_num = 6;
  }
  Uint32 index = (area_num - 1) * NUM_OF_LEVELS_PER_AREA + (level_num - 1);
  index = levels_list[index].id;
