 ...
 */
void supervisor_bricks_level::first_init() {
  game_screen->clear();
  sprites->reset();
  ...
  gameover_counter = 0;
  if (is_verbose) {
    std::cout << "supervisor_bricks_level::first_init() area_number:" << area_number
              << "level_number:" << level_number << " difficulty_level:" << difficulty_level
              << std::endl;
  }
  ...
  money_capsules->initialize(level_desc->moneys_frequency * difficulty_level, panel_score,
                             player_indicators);

  /* initialize the object which handles bonus and penalty capsules */
  power_up_capsules->initialize(level_desc->penalties_frequency * difficulty_level,
 ...
 */
void supervisor_bricks_level::init_level() {
...
Uint32 supervisor_bricks_level::main_loop() {
   ...
   */
    ...
    if (panel_score->get_bricks_counter() == 0) {
      if (count_next > 0) {
        ...
        if (count_next > 20000000 || keyboard->wait_key() || music_finished) {
  ...
  if (phase == handler_audio::LOST_PORTION && phase != audio->get_portion_music_played()) {
 ...
 */
void supervisor_bricks_level::switch_background() {
#ifdef UNDER_DEVELOPMENT
  if (keyboard->key_is_pressed(SDLK_RSHIFT) || keyboard->key_is_pressed(SDLK_LSHIFT) ||
      keyboard->key_is_pressed(SDLK_RCTRL) || keyboard->key_is_pressed(SDLK_LCTRL) ||
      keyboard->key_is_pressed(SDLK_RALT) || !keyboard->key_is_pressed(SDLK_LALT)) {
    return;
  }
 ...
 */
void supervisor_bricks_level::initialize_background(Sint32 bkg_num) {
  if (is_verbose) {
 ...
 */
void supervisor_bricks_level::draw_tilesmap() {
  if (has_background) {
    return;
  }
  display->unlock_surfaces();
