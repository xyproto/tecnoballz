 ...
 */
bool sprite_gem::enable_if_available(sprite_ball* ball) {
  if (is_enabled) {
    return false;
  }
 ...
 * @return true if the sprite was enabled, otherwise false
 */
bool sprite_gem::enable_if_available(sprite_projectile* blast) {
  if (is_enabled) {
    return false;
  }
 ...
 */
void sprite_gem::init_gem(Sint32 xcoord, Sint32 ycoord, sprite_paddle* pad) {
  is_enabled = true;
  x_coord = xcoord;
  y_coord = ycoord;
  paddle = pad;
 ...
 */
void sprite_gem::collect(Uint32 id) {
  gem_id = id;
  set_image(id);
...
const Uint32 sprite_gem::gem_random[8] = {GREY_SQUARE,   GREEN_SPHERE, YELLOW_RING,
                                          BLUE_TRIANGLE, GOLD_RHOMBUS, BRONZE_PENTAGON,
                                          GREY_SQUARE,   GREEN_SPHERE};
