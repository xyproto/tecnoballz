 ...
 * Read the texts file
 */
void sprite_display_menu::load_text_file() {
  if (texts_of_menus != NULL) {
    return;
  }
  texts_of_menus = resources->load_texts(handler_resources::TEXTS_MAIN_MENU, MAX_OF_LINES,
                                         NUM_OF_COLUMNS, 0, false);
}

/**
 * Create offscreen and perform some initializations
 */
void sprite_display_menu::first_init() {
  load_text_file();
  ...
  text_offscreen->create_surface(NUM_OF_COLUMNS * font_width, NUM_OF_ROWS * line_spacing);
 ...
 * Set a new menu
 */
void sprite_display_menu::set_current_menu_section(Uint32 current) {
 ...
 * @return exit code DO_NO_EXIT, PROGRAM_EXIT, or START_GAME
 */
Uint32 sprite_display_menu::check_events() {
      ...
      case MAIN_SECTION:
        switch (pos_y) {
          ...
          case LINE_CODE2: {
            ...
            start_input_string(10, LINE_CODE2, supervisor_main_menu::AREA_CODE_LENGTH, area_code);
          } break;
          case LINE_SORTI:
      ...
      case OPTIONS_SECTION:
        switch (pos_y) {
          case 5:
            ...
            // input players names
          case 6:
            start_input_string(24, 6, 6, handler_players::players_list[0]->get_name());
            break;
          case 7:
            start_input_string(24, 7, 6, handler_players::players_list[1]->get_name());
            break;
          case 8:
            start_input_string(24, 8, 6, handler_players::players_list[2]->get_name());
            break;
          case 9:
            start_input_string(24, 9, 6, handler_players::players_list[3]->get_name());
            break;
          case 10:
            start_input_string(24, 10, 6, handler_players::players_list[4]->get_name());
            break;
          case 11:
            start_input_string(24, 11, 6, handler_players::players_list[5]->get_name());
            break;
 ...
 * the number of lifes
 */
void sprite_display_menu::update_strings() {
  const char* source;
  char* dest;
  ...
  for (Uint32 i = 0; i < handler_players::MAX_OF_PLAYERS; i++) {
    source = handler_players::players_list[i]->get_name();
    if (source[0] != '0' || source[1] != '4' || source[2] != '0' || source[3] != '6' ||
        source[4] != '7' || source[5] != '0') {
      birth_flag = false;
    }
  }
}
 ...
 */
void sprite_display_menu::clear_text_offscreen() {
 ...
 */
void sprite_display_menu::clear_input_zone() {
  if (!is_clean_zone) {
    return;
  }
  text_offscreen->clear(0, clear_zone_xcoord, clear_zone_ycoord, clear_zone_width,
                        clear_zone_height);
}
 ...
 */
void sprite_display_menu::draw_input_cursor() {
  if (!is_clean_zone) {
    return;
  }
  ...
  unsigned char* d = (unsigned char*)text_offscreen->get_pixel_data(
      clear_zone_xcoord + xcurs * font_width, clear_zone_ycoord);
  Uint32 n = row_size;
 ...
 */
void sprite_display_menu::start_input_string(Uint32 xcoord,
                                             Uint32 ycoord,
                                             Uint32 width,
                                             char* str) {
 ...
 */
void sprite_display_menu::clear_zone_start(Uint32 xcoord,
                                           Uint32 ycoord,
                                           Uint32 width,
                                           Uint32 height) {
  clear_zone_stop();
 ...
 */
void sprite_display_menu::clear_zone_stop() {
  is_clean_zone = false;
 ...
 */
void sprite_display_menu::copy_high_score_in_menu() {
...
//------------------------------------------------------------------------------
char const sprite_display_menu::difficulte[] =
    "EASY"
    "HARD"
    "MAD "
    "DEAD";
...
const unsigned char sprite_display_menu::color_cycling[] = {
