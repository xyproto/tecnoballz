...
handler_display::~handler_display() {
  ...
  if (SDL_WasInit(SDL_INIT_VIDEO) != 0) {
    if (is_verbose) {
      std::cout << ">handler_display::~handler_display() SDL_VideoQuit()" << std::endl;
    }
    SDL_VideoQuit();
  }
  SDL_Quit();
 ...
 */
void handler_display::initialize() {
  set_video_mode();
 ...
 */
void handler_display::set_video_mode() {
  window_width = 320 * resolution;
  window_height = 240 * resolution;
  offsetplus = 64 * resolution;
  if (is_verbose) {
    std::cout << ">handler_display::set_video_mode() " << window_width << "x" << window_height
              << std::endl;
  }
 ...
 */
void handler_display::lock_surfaces() {
 ...
 */
void handler_display::unlock_surfaces() {
...
void handler_display::get_info() {
  ...
  printf("Rshift=%i Gshift=%i Bshift=%i Ashift=%i \n", vi->vfmt->Rshift, vi->vfmt->Gshift,
         vi->vfmt->Bshift, vi->vfmt->Ashift);
  printf("Rloss=%i Gloss=%i Bloss=%i Aloss=%i \n", vi->vfmt->Rloss, vi->vfmt->Gloss,
         vi->vfmt->Bloss, vi->vfmt->Aloss);
 ...
 */
void handler_display::check_if_toggle_fullscreen() {
 ...
 */
void handler_display::wait_frame() {
  frame_counter++;
 ...
 */
void handler_display::enable_palette(unsigned char* pal) {
  if (is_verbose) {
 ...
 */
void handler_display::enable_palette(SDL_Color* pal) {
  if (is_verbose) {
 ...
 */
SDL_Color* handler_display::get_palette() {
  return sdl_palette;
}
 ...
 */
void handler_display::window_update() {
  SDL_Rect source = {0, (Sint16)(offsetplus + tilt_offset), (Uint16)window_width,
                     (Uint16)(window_height + offsetplus + tilt_offset)};
  SDL_Rect destination = {0, 0, (Uint16)window_width, (Uint16)window_height};
...
//-------------------------------------------------------------------------------
void handler_display::clr_shadow(Sint32 offst, Sint32 large, Sint32 haute) {
  char zmask = 0x7F;
 ...
 * @param height
 */
void handler_display::clr_shadow(Sint32 xcoord, Sint32 ycoord, Sint32 width, Sint32 height) {
  char zmask = 0x7F;
...
//                      => haute: height box en pixels
//------------------------------------------------------------------------------
void handler_display::set_shadow(Sint32 offst, Sint32 large, Sint32 haute) {
...
//------------------------------------------------------------------------------
void handler_display::buf_affx32(char* srcPT, char* desPT, Sint32 width, Sint32 heigh) {
...
//------------------------------------------------------------------------------
void handler_display::buf_affx64(char* srcPT, char* desPT, Sint32 width, Sint32 heigh) {
 ...
 */
void handler_display::tilt_screen() {
 ...
 */
void handler_display::set_color_gradation() {
