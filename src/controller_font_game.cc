 ...
 * @param offset
 */
void controller_font_game::initialize(Uint32 level, Uint32 offset) {
  ...
  Sint32 a = set_start_values(size_of_line_1, 0, 0, yStrt, -6 * resolution, 60 * resolution);

  /* initialize characters of "COMPLETED" */
  yStrt += 10 * resolution;
  set_start_values(size_of_line_2, a, size_of_line_1, yStrt, -8 * resolution, 80 * resolution);
}
 ...
 * @return
 */
Uint32 controller_font_game::set_start_values(Uint32 length,
                                              Uint32 zeRad,
                                              Uint32 index,
                                              Sint32 yStrt,
                                              Uint32 yOffs,
 ...
 */
void controller_font_game::move() {
  for (Uint32 i = 0; i < total_size; i++) {
    sprite_font_game* sprite_char = sprites_list[i];
    sprite_char->move();
  }
 ...
 */
void controller_font_game::enable() {
  for (Uint32 i = 0; i < total_size; i++) {
    sprite_font_game* sprite_char = sprites_list[i];
    sprite_char->enable();
  }
...
// string to display
//-----------------------------------------------------------------------------
char controller_font_game::ze_bobText[] = "LEVEL[//\0COMPLETED\0";
char controller_font_game::ze_endText[] = "[[GAME[[\0FINISHED[\0";
