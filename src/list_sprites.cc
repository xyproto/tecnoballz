 ...
 */
void list_sprites::init(Uint32 numof) {
  try {
    if (NULL == shapes) {
      max_of_shapes = numof;
 ...
 */
Uint32 list_sprites::get_sprites_remaining() {
 ...
 */
void list_sprites::add(sprite_object* sprite) {
  if (num_of_shapes >= max_of_shapes - 1) {
    std::cerr << "(!)list_sprites::add maximum number of sprites " << num_of_shapes
              << "has been reached!";
    throw std::runtime_error(
        "(!)list_sprites::add maximum "
