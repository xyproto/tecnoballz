 ...
 */
short_info_messages* short_info_messages::get_instance() {
 ...
 */
void short_info_messages::initialize() {
  if (NULL == bitmap_small_fonts) {
    ...
    screen = game_screen->get_pixel_data(XCOORD_MESSAGE * resolution, YCOORD_MESSAGE * resolution);
  ...
  if (NULL == messages) {
    messages =
        resources->load_texts(handler_resources::TEXTS_MESSAGES, MAX_OF_MESSAGES, MAX_OF_CHARS, 0);
 ...
 */
void short_info_messages::init_with_background() {
  if (!has_background) {
    return;
  }

  background =
      background_screen->get_pixel_data(XCOORD_MESSAGE * resolution, YCOORD_MESSAGE * resolution);
 ...
 */
void short_info_messages::clear_messages_request() {
  for (Uint32 i = 0; i < MAX_OF_MESSAGES; i++) {
 ...
 */
void short_info_messages::send_message_request(Uint32 id) {
 ...
 */
void short_info_messages::display(Uint32 height) {
 ...
 */
void short_info_messages::display_in_background() {
 ...
 */
void short_info_messages::clear_message() {
