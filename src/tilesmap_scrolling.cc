 ...
 */
bitmap_data* tilesmap_scrolling::get_bitmap() {
  return tiles_bitmap;
}
 ...
 */
void tilesmap_scrolling::initialize(Uint32 pal_id, Uint32 map_id) {
  if (is_verbose) {
  ...
  if (is_verbose) {
    std::cout << "tilesmap_scrolling::initialise() tile_width:" << tile_width
              << "; tiles_per_row:" << tiles_per_row << std::endl;
  }
  y_coord = 0;
 ...
 * @param map_id map number identifier
 */
void tilesmap_scrolling::switch_map(Uint32 pal_id, Uint32 map_id) {
  if (NULL != map_tiles) {
    delete[] map_tiles;
    map_tiles = NULL;
  }
 ...
 * @param pal_id palette number identifier
 */
void tilesmap_scrolling::enable_palette(Uint32 pal_id) {
 ...
 * @param index
 */
void tilesmap_scrolling::scroll(Sint32 index) {
 ...
 */
bitmap_data* tilesmap_scrolling::alloc_brush(Uint16* map,
                                             Uint32 num_of_cols,
                                             Uint32 num_of_lines) {
  ...
  rect_src.w = rect_dst.w = tile_width;
  if (is_verbose) {
    std::cout << "tilesmap_scrolling::alloc_brush num_of_cols:" << num_of_cols
              << " num_of_lines:" << num_of_lines << std::endl;
  }
  rect_dst.y = 0;
  for (Uint32 v = 0; v < num_of_lines; v++) {
    ...
    for (Uint32 h = 0; h < num_of_cols; h++) {
      ...
      if (SDL_BlitSurface(tiles_surface, &rect_src, brush_surface, &rect_dst) < 0) {
        std::cerr << "(!)tilesmap_scrolling::draw_tiles() "
                  << "SDL_BlitSurface() return " << SDL_GetError() << std::endl;
      }
      rect_dst.x += tile_width;
 ...
 * @param map_id map number identifier
 */
void tilesmap_scrolling::load_map(Uint32 map_id) {
  switch (map_id) {
    case MAP_GUARDIANS:
    default:
      if (is_40_columns) {
...
const unsigned char tilesmap_scrolling::colors_map[48] = {
    2,   2,   2,  17,  33, 50, 66,  98,  130, 19,  50,  81, 27,  58,  90,  85,
    101, 133, 36, 66,  98, 39, 70,  102, 146, 146, 178, 82, 82,  114, 129, 129,
    161, 59,  90, 122, 98, 98, 130, 42,  74,  106, 50,  82, 114, 111, 114, 146};
