 ...
 */
void controller_capsules::initialize(Uint32 delay, const Uint32* random) {
  ...
  if (current_phase == BRICKS_LEVEL) {
    ...
    if (num_of_caspules_bought > 0) {
      bonus_delay = (numof_bricks - numof_bricks / 2) / num_of_caspules_bought;
    } else {
 ...
 */
void controller_capsules::send_capsule_from_brick(brick_redraw* brick) {
  ...
  for (Uint32 i = 0; i < max_of_sprites; i++) {
     ...
     */
    if (num_of_caspules_bought > capsules_released_count && bricks_breaked_count > bonus_delay) {
      bricks_breaked_count = 0;
 ...
 */
void controller_capsules::send_penalty_or_bonus(sprite_ball* ball) {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
 ...
 */
void controller_capsules::send_penalty_or_bonus(sprite_projectile* blast) {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_capsule* capsule = sprites_list[i];
    if (capsule->is_enabled) {
      continue;
 ...
 */
void controller_capsules::check_if_send_capsule(sprite_ball* ball) {
  capsule_delay_counter++;
  if (capsule_delay_counter <= capsule_delay) {
    return;
  }
 ...
 */
void controller_capsules::create_shop_sprites_list() {
  ...
  caspule = *caspules;
  caspule->set_coordinates(SHIP_XCOORD_INDICATOR * resolution, SHIP_YCOORD_INDICATOR * resolution);
  caspule->disable();
 ...
 */
void controller_capsules::set_overview_capsule(Uint32 id) {
 ...
 */
void controller_capsules::move_in_bricks_level() {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_capsule* capsule = sprites_list[i];
 ...
 */
void controller_capsules::move_in_guardians_level() {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_capsule* capsule = sprites_list[i];
    capsule->play_animation_loop();
    sprite_paddle* paddle = capsule->move();
 ...
 */
void controller_capsules::play_animation_in_shop(Uint32 speed) {
  if (--frame_delay <= 0) {
 ...
 */
void controller_capsules::check_cheat_keys() {
  if (!is_enabled_cheat_mode) {
    return;
  }
  if (keyboard->key_is_pressed(SDLK_LSHIFT) || keyboard->key_is_pressed(SDLK_LCTRL) ||
      keyboard->key_is_pressed(SDLK_RALT) ||
#ifndef UNDER_DEVELOPMENT
      !keyboard->key_is_pressed(SDLK_RSHIFT) || !keyboard->key_is_pressed(SDLK_RCTRL) ||
#else
      keyboard->key_is_pressed(SDLK_RSHIFT) || keyboard->key_is_pressed(SDLK_RCTRL) ||
#endif
 ...
 */
void controller_capsules::run_in_bricks_level(sprite_paddle* paddle, Uint32 capsule_id) {
 ...
 * @param capsule_id Option identifier
 */
void controller_capsules::run_in_guards_level(sprite_paddle* paddle, Uint32 capsule_id) {
  paddle_selected = paddle;
  controller_balls* balls = controller_balls::get_instance();
  switch (capsule_id) {
 ...
 */
const Uint16 controller_capsules::randomlist[128] = {sprite_capsule::BOTTOM_WALL,
 ...
 */
Sint16 controller_capsules::cheat_keys[] = {SDLK_F1,
                                            sprite_capsule::GLUE,
                                            0,
                                            SDLK_F2,
                                            sprite_capsule::NEXT_LEVEL,
                                            ...
                                            0,
                                            SDLK_e,
                                            sprite_capsule::ENABLE_BOTTOM_PADDLE,
                                            0,
                                            SDLK_r,
                                            sprite_capsule::ENABLE_RIGHT_PADDLE,
                                            0,
                                            ...
                                            0,
                                            SDLK_o,
                                            sprite_capsule::CHANCE,
                                            0,
                                            SDLK_q,
                                            sprite_capsule::ENABLE_HUGELY_OPTIONS,
                                            0,
                                            SDLK_s,
                                            sprite_capsule::SET_THE_PRICES_TO_1,
                                            0,
