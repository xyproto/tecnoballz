...
const char* configfile::language_to_string[MAX_OF_LANGUAGES] = {"en", "fr"};

const std::string configfile::CONF_DIR_NAME("tlk-games");
const std::string configfile::CONF_FILENAME("tecnoballz.conf");
...
void configfile::configinfo() {
  ...
  fprintf(stdout,
          ...
          "- optionfull : %i\n- is_audio_enable: %i\n- resolution:%i\n"
          "- is_verbose: %i\n difficulty_level : %i\n",
          handler_display::optionfull, audio, resolution, is_verbose, difficulty_level);
}
 ...
 */
void configfile::get_fullpathname() {
...
void configfile::load() {
  ...
  if (root_obj == NULL) {
    std::cerr << "lispreader::lisp_read_file (" << conf_filename << ") was failed" << std::endl;
    return;
  }
  ...
  if (number_of_players < 1 || number_of_players > (Sint32)handler_players::MAX_OF_PLAYERS) {
    number_of_players = 1;
  }
 ...
 */
const char* configfile::get_player_name(Uint32 playernum) {
 ...
 */
void configfile::set_player_name(Uint32 playernum, const char* name) {
  if (playernum >= handler_players::MAX_OF_PLAYERS) {
    playernum = handler_players::MAX_OF_PLAYERS - 1;
  }
 ...
 * @return "en" or fr
 */
const char* configfile::get_language() {
...
FILE* configfile::fopen_data(const char* fname, const char* fmode) {
  ...
  if (fi == NULL) {
    fprintf(stderr,
            "configfile::fopen_data(): Warning: Unable "
            "to open the file \"%s\" ",
            fname);
    if (strcmp(fmode, "r") == 0) {
...
Sint32 configfile::scan_arguments(Sint32 arg_count, char** arg_values) {
  ...
  for (i = 1; i < arg_count; i++) {
    ...
    if (!strcmp(arg_values[i], "--verbose") || !strcmp(arg_values[i], "-v")) {
      is_verbose = true;
      continue;
    }
