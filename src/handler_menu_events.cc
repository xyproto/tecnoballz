 ...
 */
void handler_menu_events::begin(Sint32 xspace,
                                Sint32 yspace,
                                Sint32 xmin,
                                Sint32 ymin,
                                Sint32 xmax,
 ...
 * @param ytop Y-coordinate top of the menu
 */
void handler_menu_events::start(Sint32 spacing,
                                Sint32 min,
                                Sint32 max,
                                Sint32 xcenter,
                                Sint32 ytop) {
  if (is_verbose) {
    std::cout << " handler_menu_events::start() "
              << " line_spacing: " << spacing << " line_min: " << min << " line_max: " << max
              << " x_center: " << xcenter << " top_y_coord: " << ytop << std::endl;
  }
  is_enabled = true;
  y_coord_left_down = handler_keyboard::NULL_YCOORD;
  y_coord_right_down = handler_keyboard::NULL_YCOORD;
...
Uint32 handler_menu_events::keys[MAX_OF_KEYS] = {
    handler_keyboard::K_UP,       handler_keyboard::K_DOWN, handler_keyboard::K_LEFT,
    handler_keyboard::K_RIGHT,    handler_keyboard::K_FIRE, handler_keyboard::K_RELEASE_BALL,
...
bool handler_menu_events::check_keys(Uint32* kcode, Uint32* prev_kcode) {
  ...
  *kcode = 0;
  ...
  if (previous_key_code_down > 0 && !keyboard->control_is_pressed(previous_key_code_down)) {
