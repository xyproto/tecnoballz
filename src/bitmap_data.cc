 ...
 */
char* bitmap_data::get_pixel_data(Sint32 xcoord, Sint32 ycoord) {
 ...
 */
void bitmap_data::create_surface(Uint32 w, Uint32 h) {
 ...
 */
char* bitmap_data::duplicate_pixel_data() {
  char* pixel;
  try {
    pixel = new char[bytes_size];
  } catch (std::bad_alloc&) {
    std::cerr << "bitmap_data::duplicate_pixel_data() "
              << "not enough memory to allocate " << bytes_size << " bytes " << std::endl;
    throw;
  }
 ...
 */
void bitmap_data::enable_palette() {
 ...
 */
unsigned char* bitmap_data::get_palette() {
  return palette;
}
 ...
 */
bitmap_data* bitmap_data::cut_to_bitmap(Sint32 xcoord, Sint32 ycoord, Uint32 w, Uint32 h) {
