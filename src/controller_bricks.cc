 ...
 */
void controller_bricks::first_init() {
 ...
 */
void controller_bricks::initialize() {
  if (is_verbose) {
  ...
  if (NULL == bitmap_bricks) {
    ...
    bitmap_bricks = bmp_bricks->cut_to_surface(bposx, bposy, bricks_height, bricks_width);
    delete bmp_bricks;

    /* replace color 0 by color 29 for color cycling */
    char* pixels = bitmap_bricks->get_pixel_data();
    Uint32 count = bitmap_bricks->get_height() * bitmap_bricks->get_width();
 ...
 */
void controller_bricks::load_level(Sint32 area_nu, Sint32 level_nu) {
  if (is_verbose) {
    std::cout << "controller_bricks::load_level() area_nu: " << area_nu << "level_nu: " << level_nu
              << std::endl;
  }
  ...
  char* level = all_levels + (SIZEOFAREA * 2 * (area_nu - 1)) + (LEVEL_SIZE * 2 * (level_nu - 1));
  ...
  for (Uint32 j = 0; j < BRICKS_MAP_HEIGHT; j++, map += 3, bobindex += 3, ycoord += brkyoffset) {
    ...
    for (Uint32 i = 0; i < BRICKS_MAP_WIDTH; i++, map++, bobindex++, xcoord += brick_width) {
      ...
      if (pos_x > 0 || pos_y > 0) {
        ...
        map->v_pos = pos_y;
        offset = bitmap_bricks->get_offset(pos_x * 8 * resolution, pos_y * brick_height);
        ...
        if (!has_background) {
          ...
          if (sprite_template == NULL) {
            sprite->create_sprite(sprite_object::BRICKS, bitmap_bricks, true, true);
 ...
 */
void controller_bricks::add_to_sprites_list() {
 ...
 */
void controller_bricks::draw_bricks_shadows() {
  if (!has_background) {
    return;
  }
  brick_info* map = bricks_map;
 ...
 * Draw all bricks
 */
void controller_bricks::draw_bricks() {
  if (!has_background) {
    return;
  }
  brick_info* map = bricks_map;
 ...
 */
void controller_bricks::draw_brick(char* pixels, Sint32 offset, Sint32 color) {
 ...
 */
void controller_bricks::color_cycling() {
  if (has_background or !is_cycling) {
    return;
  }
  brick_info* map = bricks_map;
  map += ((6 + BRICKS_MAP_HEIGHT - 9) * MAX_OF_BRICKS_HORIZONTALLY + 3) - 1;
  map = bricks_map + ((6 + BRICKS_MAP_HEIGHT - 1) * MAX_OF_BRICKS_HORIZONTALLY) + 3 +
        BRICKS_MAP_WIDTH - 1;
 ...
 */
void controller_bricks::set_bricks_palette() {
 ...
 * save background under bricks
 */
void controller_bricks::save_background() {
  Sint32* save = (Sint32*)background_under_bricks;
  brick_info* map = bricks_map;
  Uint32 offs = game_screen->get_row_size() - brick_width;
  for (Uint32 y = 0; y < MAX_OF_BRICKS_VERTICALLY * brkyoffset; y += brkyoffset) {
    for (Uint32 x = 0; x < MAX_OF_BRICKS_HORIZONTALLY * brick_width; x += brick_width, map++) {
      Sint32* screen = (Sint32*)background_screen->get_pixel_data(x, y);
      map->save_background = save;
      for (Uint32 i = 0; i < brick_height; i++, screen = (Sint32*)((char*)screen + offs)) {
 ...
 */
void controller_bricks::less_bricks() {
...
bool controller_bricks::update() {
  ...
  if ((map + offBri_DD)->is_displayed) {
    decal = display->ecran_next(adres, brick_width - shadow_offset, shadow_offset);
 ...
 */
void controller_bricks::clr_bricks() {
  brick_info* map = bricks_map;
  /* 6 first lines are always empty */
  map += (6 * MAX_OF_BRICKS_HORIZONTALLY);
 ...
 */
brick_info* controller_bricks::get_bricks_map() {
  return bricks_map;
}
 ...
 * @return a pointer to the bricks bricks_map
 */
brick_info* controller_bricks::get_bricks_map(Sint32 xcoord, Sint32 ycoord) {
  return bricks_map + (xcoord / brick_width) + (ycoord / brkyoffset * MAX_OF_BRICKS_HORIZONTALLY);
}
 ...
 */
brick_redraw* controller_bricks::get_bricks_redraw() {
 ...
 * @return a pointer to the bricks
 */
brick_redraw* controller_bricks::get_bricks_redraw_next() {
 ...
 * @return a pointer to the bricks
 */
void controller_bricks::bricks_redraw_next() {
  briqueSave += 1;
  briqueSave &= (MAXBRIKCLR - 1);
}
