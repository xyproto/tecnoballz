 ...
 */
void controller_balls::init(Uint32 start, Uint32 glue, Uint32 speed, Uint32 tilt, Uint32 table) {
 ...
 */
void controller_balls::run_in_bricks_levels() {
  activate_tilt();
  bricks_collision();
  move_balls();
 ...
 */
void controller_balls::run_in_guardians_level() {
 ...
 */
void controller_balls::check_outside_balls() {
 ...
 * Check if balls go out of the screen of game
 */
void controller_balls::check_outside_balls_guards_level() {
  Sint32 max_y = sprite_ball::Y_MAXIMUM * resolution;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = sprites_list[i];
    if (!ball->is_enabled) {
 ...
 */
void controller_balls::activate_tilt() {
 ...
 */
void controller_balls::move_balls() {
  Sint32 j;
 ...
 */
void controller_balls::move_balls_in_guards_level() {
  Sint32 j;
  sprite_paddle* paddle;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = sprites_list[i];
    ...
    if (ball->sticky_paddle_num == 0) {
      ...
      if (j > 64) {
        std::cerr << "controller_balls::"
                  << "move_balls_in_guards_level() ball->direction = " << j << std::endl;
        j = 60;
      }
      Sint16* velocities = ball->velocities;
      velocities = (Sint16*)((char*)velocities + j);
    ...
    switch (ball->sticky_paddle_num) {
      case controller_paddles::BOTTOM_PADDLE:
        j = (paddle->collision_width >> 1) - ((ball->collision_width >> 1) + 1);
        j += paddle->x_coord;
        ball->x_coord = j;
 ...
 */
void controller_balls::collisions_with_paddles() {
  Sint32 j, x, y;
  ...
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    ...
    if (NULL != touched_paddle) {
      ...
      j = ball->direction;
      if (j > 64) {
        std::cerr << "controller_balls::collisions_with_paddles() "
                  << "(1) ball->direction " << j;
        j = 64;
      }
 ...
 */
void controller_balls::collisions_with_paddle() {
  Sint32 j, x, y;
  const Sint32* monPT;
  sprite_paddle *paddle, *touched_paddle;
  paddle_bottom->is_hit_ball = false;
 ...
 */
void controller_balls::collisions_with_robot() {
  ...
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    ...
    if (ball->x_coord + (Sint32)ball->collision_width > x1 &&
        ball->y_coord + (Sint32)ball->collision_height > y1 && ball->x_coord < x2 &&
        ball->y_coord < y2) {
 ...
 */
void controller_balls::handle_ejectors() {
  controller_ejectors* ejectors = controller_ejectors::get_instance();
  sprite_ejector* eject0 = ejectors->get_ejector(controller_ejectors::TOP_LEFT_EJECTOR);
  sprite_ejector* eject1 = ejectors->get_ejector(controller_ejectors::BOTTOM_LEFT_EJECTOR);
  sprite_ejector* eject2 = ejectors->get_ejector(controller_ejectors::BOTTOM_RIGHT_EJECTOR);
  sprite_ejector* eject3 = ejectors->get_ejector(controller_ejectors::TOP_RIGHT_EJECTOR);
 ...
 */
void controller_balls::collisions_with_walls() {
 ...
 */
void controller_balls::collisions_with_sides() {
 ...
 */
void controller_balls::prevent_horizontal_blocking() {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = sprites_list[i];
    if (!ball->is_enabled) {
      continue;
 ...
 */
void controller_balls::bricks_collision() {
  controller_bricks* bricks = controller_bricks::get_instance();
 ...
 */
void controller_balls::collisions_with_eyes() {
  ...
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    ...
    for (Sint32 j = 0; j < numof_eyes; j++) {
      ...
      Sint32 delta_x = ball->x_coord + (ball->collision_width / 2) - center_x;
      delta_x = delta_x * delta_x;
      Sint32 delta_y = ball->y_coord + (ball->collision_width / 2) - center_y;
 ...
 */
void controller_balls::collisions_with_ships() {
  controller_ships* ships = controller_ships::get_instance();
  sprite_ball** balls = sprites_list;
 ...
 */
void controller_balls::collisions_with_guardians() {
 ...
 */
sprite_ball* controller_balls::get_first_ball() {
  sprite_ball** balls = sprites_list;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = *(balls++);
    if (ball->is_enabled) {
 ...
 */
void controller_balls::enable_balls_control() {
 ...
 */
void controller_balls::extra_balls() {
  add_balls(2);
}
 ...
 */
void controller_balls::add_balls(Uint32 nball) {
  if (nball < 1) {
 ...
 */
void controller_balls::multi_balls() {
 ...
 */
void controller_balls::set_power_1() {
  sprite_ball** balls = sprites_list;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = *(balls++);
    if (ball->is_enabled) {
 ...
 */
void controller_balls::set_power_2() {
  sprite_ball** balls = sprites_list;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = *(balls++);
    if (ball->is_enabled) {
 ...
 */
void controller_balls::set_size_2() {
  sprite_ball** balls = sprites_list;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = *(balls++);
    if (ball->is_enabled) {
 ...
 */
void controller_balls::set_size_3() {
  sprite_ball** balls = sprites_list;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = *(balls++);
    if (ball->is_enabled) {
 ...
 */
void controller_balls::set_maximum_speed() {
  sprite_ball** balls = sprites_list;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_ball* ball = *(balls++);
    if (ball->is_enabled) {
 ...
 */
void controller_balls::check_tilt_availability() {
 ...
 */
void controller_balls::controll_balls() {
 ...
 */
Sint32 controller_balls::rb0[16] = {64, 64, 64, 64, 64, 64, 64, 64,
                                    64, 64, 64, 64, 64, 64, 64, 64};

/* right rebound */
Sint32 controller_balls::rb1[16] = {32, 28, 24, 20, 20, 24, 24, 28,
                                    32, 36, 40, 40, 44, 44, 40, 36};

Sint32 controller_balls::rb2[16] = {48, 36, 40, 44, 32, 44, 24, 28,
                                    32, 36, 40, 44, 48, 48, 44, 40};

/* top rebond */
Sint32 controller_balls::rb3[16] = {60, 60, 56, 52, 48, 44, 40, 36,
                                    36, 40, 40, 44, 48, 52, 56, 56};

Sint32 controller_balls::rb4[16] = {0, 4, 8, 0, 0, 52, 56, 60, 48, 52, 56, 44, 48, 52, 56, 60};

/* left rebond */
Sint32 controller_balls::rb5[16] = {0, 4, 8, 8, 12, 12, 8, 4, 0, 60, 56, 52, 52, 36, 56, 60};

Sint32 controller_balls::rb6[16] = {0, 4, 8, 12, 16, 20, 24, 12, 16, 12, 8, 4, 0, 4, 8, 60};

/* bottom rebond */
Sint32 controller_balls::rb7[16] = {4, 8, 12, 12, 16, 20, 20, 24, 28, 28, 24, 20, 16, 12, 8, 4};

Sint32 controller_balls::rb8[16] = {16, 20, 24, 12, 16, 20, 24, 28,
                                    32, 36, 40, 28, 32, 20, 24, 28};

Sint32* controller_balls::brick_jump[15] = {rb1, rb3, rb2, rb5, rb1, rb4, rb3, rb7,
                                            rb8, rb2, rb1, rb6, rb7, rb5, rb1};
