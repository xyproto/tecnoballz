 ...
 */
void supervisor_map_editor::first_init() {
  try {
    map_selection = new selected_region;
    tiles_selection = new selected_region;
  } catch (std::bad_alloc&) {
    std::cerr << "(!)supervisor_map_editor::first_init() "
 ...
 */
void supervisor_map_editor::view_map_editor() {
  current_selection = map_selection;
 ...
 */
void supervisor_map_editor::map_to_brush() {
  if (is_verbose) {
    std::cout << "supervisor_map_editor::map_to_brush() (" << current_selection->x1 << ", "
              << current_selection->y1 << "," << current_selection->x2 << ", "
              << current_selection->y2 << std::endl;
  }
 ...
 */
void supervisor_map_editor::tiles_to_brush() {
  if (is_verbose) {
    std::cout << "supervisor_map_editor::map_to_brush() (" << current_selection->x1 << ", "
              << current_selection->y1 << "," << current_selection->x2 << ", "
              << current_selection->y2 << std::endl;
  ...
  Sint32 offset =
      (current_selection->y1 / tile_width) * width + (current_selection->x1 / tile_width);
  Uint16* brush = tiles_brush;
  for (Uint32 y = 0; y < current_selection->number_of_raws; y++) {
    Sint32 index = offset;
    for (Uint32 x = 0; x < current_selection->number_of_cols; x++) {
 ...
 */
void supervisor_map_editor::select_rectangle() {
  ...
  if (is_right_down && !is_right_button_down) {
    ...
    current_selection->y1 = current_selection->y_offset + keyboard->get_mouse_y();
  ...
  if (is_right_button_down) {
    ...
    current_selection->y2 = keyboard->get_mouse_y() + current_selection->y_offset;
    ...
    if (current_selection->y1 > current_selection->y2 &&
        current_selection->y1 - current_selection->y2 > (screen_height / 2)) {
      current_selection->y2 = current_selection->y1 - (screen_height / 2);
    }
  }
  ...
  current_selection->number_of_cols = (current_selection->x2 - current_selection->x1) / tile_width;
  current_selection->number_of_raws = (current_selection->y2 - current_selection->y1) / tile_width;
  switch (view_mode) {
    case SHOW_TILES:
      tiles_to_brush();
      break;
 ...
 */
void supervisor_map_editor::highlight_selection() {
  char* screen;
 ...
 */
void supervisor_map_editor::alloc_tilesmap_brush(Uint32 number_of_raws, Uint32 number_of_cols) {
  if (NULL != tiles_brush) {
    delete[] tiles_brush;
  }
  ...
  try {
    ...
    std::cerr << "(!)supervisor_map_editor::alloc_tilesmap_brush() "
                 "not enough memory to allocate "
 ...
 */
void supervisor_map_editor::alloc_brush() {
  if (NULL != brush_bitmap) {
    delete brush_bitmap;
  }
  brush_bitmap = tiles_map->alloc_brush(tiles_brush, current_selection->number_of_cols,
 ...
 */
void supervisor_map_editor::draw_brush() {
  if (NULL == brush_bitmap) {
    return;
  }
  ...
  if (is_left_down && is_left_button_down) {
    ...
    if (!is_left_down && is_left_button_down) {
      ...
      Uint16* t_end = tiles_map->map_tiles + (tilesmap_scrolling::MAP_HEIGHT * map_width);
      ...
      Sint32 tsupp = (display->get_height() / tiles_map->tile_height) * 2;
      for (Uint32 j = 0; j < (tsupp * map_width); j++) {
        tiles_map->map_tiles[i++] = table[j];
      }
    }
  }
  game_screen->blit_surface(brush_bitmap, 0, 0, xcoord, ycoord - (map_ycoord & tile_mask2),
                            brush_bitmap->get_width(), brush_bitmap->get_height());
}
...
bool supervisor_map_editor::save_tilesmap() {
  ...
  */
  ...
  umask(0002);
  const char* filename = "edmap.data";
  Sint32 handle = open(filename, O_WRONLY | O_CREAT, 00666);
  if (-1 == handle) {
    std::cerr << "supervisor_map_editor::save_tilesmap() file " << filename << "; error "
  ...
  delete[] filedata;
  if (is_verbose) {
    std::cout << "supervisor_map_editor::save_tilesmap() " << filename << "file was saved"
              << std::endl;
  }
  return true;
}

const unsigned char supervisor_map_editor::cycled_colors_list[MAX_OF_CYCLED_COLORS] = {
