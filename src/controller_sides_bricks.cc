 ...
 */
void controller_sides_bricks::initialize() {
  if (is_verbose) {
  ...
  horizontal_brick->create_sprite(sprite_object::HORIZONTAL_SIDE_BRICK, sprites_bitmap, true);
  ...
  vertical_brick->create_sprite(sprite_object::VERTICAL_SIDE_BRICK, sprites_bitmap, true);
 ...
 */
void controller_sides_bricks::create_bricks_sprites() {
  if (has_background) {
    return;
  }
 ...
 */
void controller_sides_bricks::save_state_of_walls() {
  bool* left = current_player->get_map_left();
  bool* right = current_player->get_map_right();
  bool* top = current_player->get_map_top();
  for (Uint32 i = 0; i < MAX_OF_SIDES_BRICKS; i++) {
 ...
 */
void controller_sides_bricks::save_background() {
  background_top_side = background_screen->cut_to_surface(
      xcoord_top_wall, ycoord_top_wall, horizontal_brick_width * MAX_OF_SIDES_BRICKS,
      horizontal_brick_height);
  background_right_side =
      background_screen->cut_to_surface(xcoord_right_wall, ycoord_right_wall, vertical_brick_width,
                                        vertical_brick_height * MAX_OF_SIDES_BRICKS);
  background_left_side =
      background_screen->cut_to_surface(xcoord_left_wall, ycoord_left_wall, vertical_brick_width,
                                        vertical_brick_height * MAX_OF_SIDES_BRICKS);
}
 ...
 */
void controller_sides_bricks::run() {
  if (has_background) {
 ...
 */
void controller_sides_bricks::restore() {
 ...
 */
void controller_sides_bricks::disable_sprites() {
  for (Uint32 i = 0; i < MAX_OF_SIDES_BRICKS; i++) {
 ...
 */
void controller_sides_bricks::draw_shadows_to_brackground() {
  if (!has_background) {
    return;
  }
  Sint32 xcoord_top = xcoord_top_wall;
 ...
 */
void controller_sides_bricks::draw_to_brackground() {
  if (!has_background) {
    return;
  }
  Sint32 xcoord_top = xcoord_top_wall;
...
bool controller_sides_bricks::collision_with_top_wall(Sint32 xcoord) {
  ...
  Sint32 index =
      (Sint32)((Sint32)(xcoord - (Sint32)xcoord_top_wall)) / (Sint32)horizontal_brick_width;
  if (index < 0 || index >= (Sint32)MAX_OF_SIDES_BRICKS) {
    return true;
  }
