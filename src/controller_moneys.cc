 ...
 * @param money
 */
void controller_moneys::initialize(Uint32 delay,
 ...
 */
void controller_moneys::send_money_from_brick(brick_redraw* briPT) {
 ...
 */
void controller_moneys::send_money(sprite_ball* ball) {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_money* money = sprites_list[i];
 ...
 */
void controller_moneys::send_money(sprite_projectile* blast) {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_money* money = sprites_list[i];
 ...
 * @param delay time delay before sending a new money capsule
 */
void controller_moneys::initialize(Uint32 delay, controller_indicators* money) {
  send_delay = delay;
  ptPrntmney = money;
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_money* money = sprites_list[i];
 ...
 */
void controller_moneys::send_money_from_guardian(sprite_ball* ball) {
  if (++delay_count <= send_delay) {
    return;
  }
  delay_count = 0;
 ...
 * in guardians levels
 */
void controller_moneys::move_bottom() {
  for (Uint32 i = 0; i < max_of_sprites; i++) {
    sprite_money* money = sprites_list[i];
    money->play_animation_loop();
