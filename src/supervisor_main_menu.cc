 ...
 */
void supervisor_main_menu::first_init() {
  if (is_verbose) {
  ...
  tecnoballz_logo->create_sprite(sprite_object::TECNOBALLZ_LOGO, sprites_bitmap, 1);
  ...
  tiles_map->initialize(tilesmap_scrolling::TILES_COLOR_MENU, tilesmap_scrolling::MAP_MENU);
 ...
 */
void supervisor_main_menu::move_tecnoballz_logo() {
  tecnoballz_logo_angle = (tecnoballz_logo_angle + 6) & SINUS_MASK;
  /* convert unsigned to signed */
  Sint32 res = (Sint32)resolution;
  Uint32 a = tecnoballz_logo_angle;
  tecnoballz_logo->set_x_coord(((table_sinL[a] * 20 * res) >> SINUS_DECA) + 32 * res);
  if (!birth_flag) {
    return;
  }
  a = (a * 4) & SINUS_MASK;
  tecnoballz_logo->set_y_coord(((table_sinL[a] * 5 * res) >> SINUS_DECA) + 7 * res);
}
 ...
 */
const char* supervisor_main_menu::get_area_code(Uint32 area_num, Uint32 difficulty) {
  if (area_num < 2) {
    return NULL;
  }
 ...
 */
char* supervisor_main_menu::get_current_area_code() {
 ...
 */
void supervisor_main_menu::copy_current_area_code(char* destination) {
